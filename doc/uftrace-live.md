...
:   Do not show small functions under the time threshold.  If some functions
...
:   Allow running uftrace even if some problems occur.  When `uftrace record`
    ...
    it quits with an error message since uftrace can not trace the executable.
...
In the above example, you can omit the function `b()` and all calls it makes with
the `-N` option.
...
function `c()` was omitted.  Note that the `-D` option works with `-F`.
...
The `traceon` and `traceoff` actions (the `_` can be omitted from `trace_on` and
